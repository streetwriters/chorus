...
type notesnookPresignedURLs struct {
	Get        string
	Put        string
	UploadPart string
}

func getNotesnookPresignedURLs(t *testing.T, endpoint, bucket, getKey, putKey, multipartKey, uploadID string) notesnookPresignedURLs {
	t.Helper()
	if _, err := exec.LookPath("dotnet"); err != nil {
		t.Skip("dotnet SDK is required for the AWSSDK.S3 presign regression")
	}
	args := []string{
		"run", "--project", "presign_notesnook/NotesnookPresign.csproj", "--verbosity", "quiet", "--",
		endpoint, storageCreds.AccessKeyID, storageCreds.SecretAccessKey, bucket,
		getKey, putKey, multipartKey, uploadID,
	}
	command := exec.CommandContext(t.Context(), "dotnet", args...)
	output, err := command.CombinedOutput()
	require.NoError(t, err, string(output))
	for _, line := range strings.Split(string(output), "\n") {
		if strings.HasPrefix(line, "NOTESNOOK_URLS:") {
			var urls notesnookPresignedURLs
			require.NoError(t, json.Unmarshal([]byte(strings.TrimPrefix(line, "NOTESNOOK_URLS:")), &urls))
			return urls
		}
	}
	t.Fatalf("AWSSDK helper did not return URLs: %s", output)
	return notesnookPresignedURLs{}
}
...
func Test_e2e_proxy_presigned_sigv4(t *testing.T) {
	...
	t.Run("presigned_multipart_upload_part", func(t *testing.T) {
		r := require.New(t)
		ctx := t.Context()
		obj := "presigned-multipart.bin"
		...
		params.Set("partNumber", strconv.Itoa(1))
		presignedURL, err := proxyClient.PresignHeader(ctx, http.MethodPut, bucket, obj, presignExpiry, params, nil)
		r.NoError(err)

		code, headers, body := doRequest(t, http.MethodPut, presignedURL, "multipart part via presigned URL")
		r.Equal(http.StatusOK, code, body)
		r.NotEmpty(headers.Get("ETag"))
		r.NoError(core.AbortMultipartUpload(ctx, bucket, obj, uploadID))
	})

	t.Run("notesnook_awssdk_3_7_310_8_presigned_get_put_upload_part", func(t *testing.T) {
		r := require.New(t)
		ctx := t.Context()
		getKey, putKey, multipartKey := "notesnook-get.txt", "notesnook-put.txt", "notesnook-part.bin"
		getBody := "presigned GET generated by AWSSDK.S3 3.7.310.8"
